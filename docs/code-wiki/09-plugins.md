...
- **内建任务插件**（`plugins/tasks/`）：随二进制分发，每个平台一个 `plugin.js`：
  kling、jimeng、hailuo、sunoapi、sora、vertex-ai、vidu。
...
[plugins/](../../../plugins/) 下的 `*_test.go`（如 vidu_responses_test.go、veo_poll_test.go、vertex_ai_responses_test.go）对内置插件的协议转换与响应解析做 Go 侧回归测试。