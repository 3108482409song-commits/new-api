...
├── channel/                 # 每个上游厂商一个 adaptor 包（40+ 个）
│   └── task/                # 异步任务平台 adaptor（kling/suno/midjourney…）
├── common/                  # RelayInfo、计费结算器、流式工具、请求转换
├── common_handler/          # 可复用 handler 模版
...
[TaskAdaptor](../../../relay/channel/adapter.go#L36-L81) 是异步任务平台（Kling/Jimeng/Hailuo/Suno/Sora/VertexAI/Vidu/Midjourney）的契约，额外包含：
...
| ali / baidu / baidu_v2 / zhipu / zhipu_4v / xunfei / ai360 / tencent / moonshot / deepseek / minimax / lingyiwanwu / volcengine / mokaai /jina / siliconflow / cohere / mistral / xai / perplexity / openrouter / ollama / xinference / coze / dify / palm / replicate / sub2api / submodel / newapi | 国内与开源/聚合平台 |
| task（channel/task/）| 异步任务平台 adaptor（kling / jimeng / hailuo / suno / sora / vertex-ai / vidu / midjourney …) |
...
| 文件 | 职责 |
| --- | --- |
| [relay_handler.go](../../../relay/relay_handler.go)（Text 等通用逻辑；各 handler 归口） | 文本请求主流程：转换 → 转发 → 流式回写 → usage 核算 → 结算 |
| text_handler.go / claude_handler.go / gemini_handler.go | 文本/Claude/Gemini 处理（Claude 与 Chat 的流式互转、Gemini 批量 embedding 等） |
